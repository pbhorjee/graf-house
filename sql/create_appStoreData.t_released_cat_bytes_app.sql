...
CREATE FUNCTION BYTE_SIZED AS(Size) -> multiIf(notEmpty(extract(Size, '(\d+\.?\d+)(?:M)')),
...
CREATE FUNCTION DATETIME_FROM_GOOG_DATE AS(Released) -> parseDateTimeBestEffortOrZero(concat(
        splitByNonAlpha(Released)[2], '-', splitByNonAlpha(Released)[1], '-', splitByNonAlpha(Released)[3]));
...
WHERE Category IN ('Games', 'Music', 'Health')
  AND Size_Bytes > 0
UNION ALL
SELECT DATETIME_FROM_GOOG_DATE(Released) AS Released, `App Id` AS App_Id, `App Name` AS App_Name,
       CAT_REPLACE(Category) AS Category, BYTE_SIZED(Size) AS Size_Bytes, Rating AS Average_Rating,
